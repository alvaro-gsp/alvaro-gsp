### Hey there, I'm Alvaro Garcia

# Senior Cloud & DevOps Engineer

I design, build, and operate multi-cloud infrastructure at enterprise scale — **200+ Azure subscriptions, 130+ AWS accounts, and 20+ GCP projects** across EMEA, APAC, and the Americas.

Early adopter of **AI-accelerated engineering**: I designed and deployed a responsible AI coding platform with private LLM models, Entra ID auth, network isolation, and cost governance — now used company-wide.

---

### Cloud Platforms

<p>
  <a href="#"><img src="https://skillicons.dev/icons?i=azure" alt="Azure" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=aws" alt="AWS" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=gcp" alt="GCP" width="48" /></a>
</p>

### Infrastructure as Code & Automation

<p>
  <a href="#"><img src="https://skillicons.dev/icons?i=terraform" alt="Terraform" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=ansible" alt="Ansible" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=helm" alt="Helm" width="48" /></a>
</p>

### Containers & Orchestration

<p>
  <a href="#"><img src="https://skillicons.dev/icons?i=kubernetes" alt="Kubernetes" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=docker" alt="Docker" width="48" /></a>
</p>

### CI/CD

<p>
  <a href="#"><img src="https://skillicons.dev/icons?i=azure" alt="Azure DevOps" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=githubactions" alt="GitHub Actions" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=jenkins" alt="Jenkins" width="48" /></a>
</p>

### Languages & Scripting

<p>
  <a href="#"><img src="https://skillicons.dev/icons?i=python" alt="Python" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=bash" alt="Bash" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=powershell" alt="PowerShell" width="48" /></a>&nbsp;&nbsp;
  <a href="#"><img src="https://skillicons.dev/icons?i=go" alt="Go" width="48" /></a>
</p>

### AI & LLM Integration

![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)
![Claude](https://img.shields.io/badge/Anthropic_Claude-D97757?style=for-the-badge&logo=anthropic&logoColor=white)
![Azure AI Foundry](https://img.shields.io/badge/Azure_AI_Foundry-0078D4?style=for-the-badge&logo=microsoftazure&logoColor=white)

---

### What I do

- **Terraform at scale** — Custom modules, landing zones, 4-repo architecture, multi-environment pipelines
- **AI-Accelerated DevOps** — Private LLM platform, AI-powered chatbots, LLM-assisted IaC development
- **Containers & Kubernetes** — AKS/EKS/GKE management, ephemeral CI/CD agents on Container Apps
- **Cloud Security & Compliance** — IAM, RBAC, WAF, secrets management, security hardening
- **FinOps** — Cost governance, right-sizing, spend anomaly detection
- **Custom Tooling** — Python CLI tools for Terraform workflows, cloud inventory, and automation

---

### Certifications

| Certification | Issuer |
|---|---|
| **Azure Solutions Architect Expert** (AZ-305) | Microsoft |
| **Azure Administrator Associate** (AZ-104) | Microsoft |
| **Azure Security Engineer Associate** (AZ-500) | Microsoft |

---

### Let's connect

[![CV](https://img.shields.io/badge/CV-alvaro--gsp.github.io-0078D4?style=for-the-badge&logo=googlechrome&logoColor=white)](https://alvaro-gsp.github.io)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-alvarogsp-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/alvarogsp)
[![Email](https://img.shields.io/badge/Email-alvarogsp@hotmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:alvarogsp@hotmail.com)
